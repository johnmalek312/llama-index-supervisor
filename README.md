...
-   This memory buffer is passed down to agents during handoffs (potentially copied depending on `output_mode`).
...
# Check the final memory
final_memory = await ctx.get("memory")
# print(final_memory.get_all())
```